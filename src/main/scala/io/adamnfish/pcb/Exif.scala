...
/** Not currently used, but this would be useful for more advanced photo
  ...
  */
object Exif {
  val tzOffsetTagInfo = new TagInfoAscii(
    "OffsetTimeOriginal",
    0x9011,
    8,
  ...
  def readPixelDate(file: File): Either[String, ZonedDateTime] = {
    ...
    m match {
      case jpegMetadata: JpegImageMetadata =>
        val dateField = jpegMetadata.getExif.findField(
          ExifTagConstants.EXIF_TAG_DATE_TIME_ORIGINAL
        )
        val tzOffsetField = jpegMetadata.getExif.findField(tzOffsetTagInfo)
        val dateTimeStr =
          s"${dateField.getValue.asInstanceOf[String]}${tzOffsetField.getValue.asInstanceOf[String]}"
        println(dateTimeStr)
        val imageTime = ZonedDateTime.parse(
          dateTimeStr,
